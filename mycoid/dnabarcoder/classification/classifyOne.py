...
def main():
	parser=argparse.ArgumentParser(prog='classifyOne.py',  
								   usage="%(prog)s [options] -i sequence -r referencefastafilename -c classificationfile -ml minalignment -cutoffs cutoffsfile -o output",
	...
	parser.add_argument('-r','--referencepath', help='The path to the folder containing reference datasets and their associated similarity cutoffs files.')
	...
	classification_result=classifyAgainstReferenceSet(sequence,referencepath,args.idcolumnname,outputpath,nproc)
...
def classifyOneSequence(sequence,referencefastafilename,classificationfilename,idcolumnname,cutoffsfilename,globalcutoff,globalconfidence,mincoverage,minseqno,mingroupno,classificationrank,outputpath,nproc):
...
def classifyAgainstReferenceSet(sequence,referencepath,idcolumnname,outputpath,nproc):
	...
	for reference in referencedict.keys():
		...
		globalconfidence=0
		classificationrank=""
		bestmatchdict=classifyOneSequence(sequence,referencefilename,classificationfilename,idcolumnname,cutoffsfilename,globalcutoff,globalconfidence,mincoverage,minseqno,mingroupno,classificationrank,outputpath,nproc)
