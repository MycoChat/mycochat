...
def BlastOutput2BestMatch(blastoutput,mincoverage):
	bestscoredict={}
	#read blast output
	blastoutputfile = open(blastoutput)
	refid = ""
	...
	for line in blastoutputfile:
		...
		if coverage < mincoverage:
			score=float(score * coverage)/mincoverage
		if not (queryid in bestscoredict.keys()):
			bestscoredict.setdefault(queryid,["",0,0,0])
		if score > bestscoredict[queryid][refid][1]:
			bestscoredict[queryid]=[refid,score,sim,coverage]
